...
def check_mandatory_fields(pat_fie, pat_log, dfr_res, wri_csv):
    """Strukturprotokoll: check Kopfdaten (pull-down) - see fields.csv"""
    ...
    for ind, row in dfr.iterrows():
        # print(str(dfr_res.loc[row['id'], 'result']))
...
def check_values(pat_log, dfr_res, wri_csv):
    """Rename strings (Geländeneigung), extract pH-Wert (Bemerkungen), """
    auf_dat = dfr_res.loc['auf_dat', 'result']
    dfr_res.loc['auf_dat', 'result'] = auf_dat.strftime("%d.%m.%Y")
    # Geländeneigung
    match dfr_res.loc['gel_nei', 'result']:
        case 'schroff':
            dfr_res.loc['gel_nei', 'result'] = 'schroff (90°-30°)'
        case 'stark':
            dfr_res.loc['gel_nei', 'result'] = 'stark (30°-15°)'
        case 'mässig':
            dfr_res.loc['gel_nei', 'result'] = 'mässig (15°-5°)'
        case 'schwach':
            dfr_res.loc['gel_nei', 'result'] = 'schwach (5°-0°)'
        case _:
...
def check_single_choice(pat_fie, pat_log, dfr_res, wri_csv, que_n_b):
...
def analyse_protocol_xls(pat_inp, pat_fie, pat_log, dir_che, dfr_fie):
    ...
    dfr_res, wri_csv = check_single_choice(pat_fie, pat_log, dfr_res, wri_csv, que_n_b)
...
def write_struktur_csv(pat_str, dfr_str, dfr_fie, dfr_res):
    ...
    new_row = pd.Series({dfr_fie.loc['que_idn', 'wiski_1']: dfr_res.loc['que_idn', 'result'],
                         dfr_fie.loc['auf_bea', 'wiski_1']: dfr_res.loc['auf_bea', 'result'],
                         ...
                         dfr_fie.loc['ver_qko', 'wiski_1']: dfr_res.loc['ver_typ', 'result'],
                         ...
                         dfr_fie.loc['bew_sta', 'wiski_1']: dfr_res.loc['bew_typ', 'result'],
                         dfr_fie.loc['ges_sta', 'wiski_1']: dfr_res.loc['ges_typ', 'result'],
                         dfr_fie.loc['rev_obj', 'wiski_1']: dfr_res.loc['rev_obj', 'result'],
                         dfr_fie.loc['n_b_k_a', 'wiski_1']: dfr_res.loc['qnb_typ', 'result']})
...
lis_xls = get_list_of_input_files(pat_log, dir_inp, ext_xls)
cou_csv = 0
...
# input file 1202-5015 Gandstafel 1_test.xlsx -> csv wird geschrieben - ok?

# kk struktur letzter Eintrag kein Abfluss um 1 verschoben - Bemerkungen passt noch
# kein Abfluss /qu zerstört ist kein single choice sonder es darf nicht angekreuzt sein oder max
# 1 Entrag, dh. pulldownmenü mit nichts oder max. 1 Eintrag (nicht 2) - d.h. dies prüfen !!!
